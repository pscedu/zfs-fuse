...
int
cmd_mount_req(int sock, zfsfuse_cmd_t *cmd)
{
...
#endif
		uint32_t ret_m = do_mount(spec, dir,
		    cmd->cmd_u.mount_req.mflag, opt);
...
handle_connection(int sock)
{
	...
	while (zfsfuse_socket_read_loop(sock, &cmd,
		...
		case IOCTL_REQ:
			...
			int ioctl_ret = zfsdev_ioctl(dev,
			    cmd.cmd_u.ioctl_req.cmd, (uintptr_t)
			    cmd.cmd_u.ioctl_req.arg, 0, &cr, NULL);
...
static ioctl_queue_item_t *
zfsfuse_ioctl_queue_find(int free/*bool*/)
{
	int i;
...
static void *
zfsfuse_ioctl_queue_worker_thread(void* init)
{
...
int
zfsfuse_ioctl_queue_init(queue_t* queue)
{
	ASSERT(queue);
	...
	for (i=0; i<IOCTLQUEUE_WORKERS; i++)
		if (pthread_create(&worker, &attr,
		    &zfsfuse_ioctl_queue_worker_thread, (void *) queue)
		    != 0)
			return -1;
...
int
zfsfuse_ioctl_queue_fini(queue_t* queue)
{
	if (!queue)
		return 0;
	...
	while (queue->active && retcode != ETIMEDOUT) {
		...
		retcode = pthread_cond_timedwait(&ioctl_queue.handled,
		    &ioctl_queue.lock, &timeout);
	}
...
int
cmd_listener_init(void)
{
	return zfsfuse_ioctl_queue_init(&ioctl_queue);
}

int
cmd_listener_fini(void)
{
#   ifdef DEBUG
