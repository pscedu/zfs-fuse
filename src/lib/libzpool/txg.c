...
int zfs_txg_timeout = 10;	/* max seconds worth of delta per txg */
...
txg_init(dsl_pool_t *dp, uint64_t txg)
{
	...
	cv_init(&tx->tx_slash2_cv2, NULL, CV_DEFAULT, NULL);
...
txg_sync_thread(dsl_pool_t *dp)
{
	...
	for (;;) {
		...
		while ((dp->dp_scan->scn_phys.scn_state != DSS_SCANNING ||
			...
			dprintf("waiting; tx_synced=%"PRIu64" waiting=%"PRIu64" dp=%p\n",
...
txg_quiesce_thread(dsl_pool_t *dp)
{
	...
	for (;;) {
		 ...
		 */
		dprintf("quiesce done, handing off txg %"PRIu64"\n", txg);
