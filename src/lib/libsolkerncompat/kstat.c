...
static dir_t *
add_dir(dir_t *root, const char *name)
{
...
static dir_t *
find_dir(dir_t *dir, fuse_ino_t inode)
{
...
kstat_create(const char *module, int instance, const char *name,
    const char *class, uchar_t type, uint_t ndata, uchar_t ks_flag)
{
	if (no_kstat_mount)
		return NULL;
	if (!root) {
		root = calloc(1, sizeof(dir_t));
		if (!root)
			return NULL;
		root->name = "/";
		root->inode = 1;
	...
	kstat_t *kstat = calloc(1, sizeof(kstat_t));
	if (kstat) {
		kstat->ks_crtime = gethrtime(); // useful ?
		kstat->ks_private = dir;
		kstat->ks_kid = dir->inode;
		dir->nb_files = ndata;
		next_inode += ndata; // reserve inodes for future files of this dir
		used_files += ndata;
		dir->files = calloc(ndata, sizeof(kstat_named_t *));
	}

	return (kstat);
}

static void mount_kstat(void);
...
static void
get_value(dir_t *dir, fuse_ino_t ino)
{
...
static int
kstat_stat(fuse_ino_t ino, struct stat *stbuf)
{
...
static void
kstat_ll_getattr(fuse_req_t req, fuse_ino_t ino,
...
static void
kstat_ll_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
...
{
	if (off < bufsize)
		return fuse_reply_buf(req, buf + off, min(bufsize - off,
		    maxsize));
	return fuse_reply_buf(req, NULL, 0);
}

static void
kstat_ll_readdir(fuse_req_t req, fuse_ino_t ino, size_t size,
...
static void
kstat_ll_open(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi)
{
...
static void
kstat_ll_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off,
    struct fuse_file_info *fi)
{
	(void) fi;
...
// prototype here then
#ifdef ZFS_SLASHLIB
#define zfsfuse_newfs pscfs_fuse_newfs
#endif
